...
app.post('/api/persons', (request, response, next) => {
  ...
  if (!newPerson.name || !newPerson.number) { return response.status(400).json({ error: 'content missing' }) }

  Person.find({})
    .then((persons) => {
      if (persons.find((person) => person.name === newPerson.name)) { return Promise.reject(new Error('name must be unique')) }
    })
    .then(() => {
